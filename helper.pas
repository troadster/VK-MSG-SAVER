...
function QiwiAuth(Login, Password: string): string;
{
  ...
  HTTP.Request.CustomHeaders.Add(QiwiAuth(Edit1.Text,Edit2.Text));
}
...
Begin
  ...
  try
    HTML := http.Post('https://sso.qiwi.com/cas/tgts', Data);
    Ticket := Pars('ticket":"', HTML, '"');
    Data.Clear;
    Data.WriteString('{"ticket":"' + Ticket +
      '","service":"https://qiwi.com/j_spring_cas_security_check"}');
    HTML := http.Post('https://sso.qiwi.com/cas/sts', Data);
    Ticket := Pars('ticket":"', HTML, '"');
    html:=http.Get('https://qiwi.com/j_spring_cas_security_check?ticket=' + Ticket);
    if Pos('+79183351464', HTML) <> 0 then
      Result :='Cookie:'+Copy(http.Request.RawHeaders.text,
